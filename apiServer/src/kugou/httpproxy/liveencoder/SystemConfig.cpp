...
{
Poco::SingletonHolder<SystemConfig> SystemConfig::Singleton;
...
void SystemConfig::initConfigDesc(program_options::options_description& desc)
{
	...
	//system parameter
    ("SYS.isScheduleServer", value2(bool,        &m_isScheduleServer),   "Is Schedule Server or not")
	("SYS.srcStreamServer", value2(string,      &m_srcStreamServer)->default_value(""),     "Source Stream Server")
	("SYS.dstStreamServer", value2(string,      &m_dstStreamServer)->default_value(""),     "Destination Stream Server")

    ("COMMUNICATION.ScheduledServerIP", value2(string,      &m_scheduleServerIP),     "Scheduled server listen this IP for communication")
	("COMMUNICATION.ScheduledServerPort",  value2(PortInt,     &m_scheduledServerPort), "Scheduled server listen this PORT for communication")  

    ("COMMUNICATION.EncoderLocalIP", value2(string,      &m_encoderLocalIP)->default_value(""),     "Encoder Client use this IP to connect scheduled server")
    ("COMMUNICATION.EncoderLocalPort",  value2(PortInt,     &m_encoderLocalPort)->default_value(0), "Encoder Client use this PORT connect scheduled server")
    ("COMMUNICATION.WebBackendDomin", value2(string,      &m_webBackendDomin)->default_value(""),     "It use to callback to web backendr")
